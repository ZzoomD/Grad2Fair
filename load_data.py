...
class FairDataset:
    ...
    def load_data(self, sens_attr=None, label_number=None, split_ratio=None, val_idx=True, sens_number=None):
        ...
        (self.adj, self.edge_index, self.edge_index_nor, self.features, self.labels, self.idx_train, self.idx_val, self.idx_test,
         self.sens) = (adj, edge_index_spar, edge_index, features, labels, idx_train, idx_val, idx_test, sens)


def dataloader(path, name, train_ratio):
    labels = np.loadtxt(f'{path}/{name}/{name}_label.txt', dtype=int)
    sens = np.loadtxt(f'{path}/{name}/{name}_sens.txt', dtype=int)
    features = np.loadtxt(f'{path}/{name}/{name}_feat.csv', delimiter=',')
    edges = np.loadtxt(f'{path}/{name}/{name}_edges.txt', delimiter=',', dtype=int)
